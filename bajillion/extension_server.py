...
class SearchServer:
    ...
    def handle_request(self, request):
        ...
        if request.command == 'search':
            ...
            # lst = [{"title": "Podcasts mark rise of DIY radio"}, {"title": "Europe backs digital TV lifestyle"}, {"title": "Learning to love broadband"}]
            
            dict_query = request.get_params()    #return a dict
            query = ""
            for key in dict_query:  #convert to strings
                query += dict_query[key] + " "

            filenames = textfiles_in_dir(DIRECTORY)
            index = {}
            file_titles = {}
            index, file_titles = create_index(filenames, index, file_titles)
            results = search(index, query)
            
            lst = [] #list of dicts including {'title': results}
